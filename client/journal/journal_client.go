...
// New creates a new journal API client with basic auth credentials.
...
// New creates a new journal API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) JournalPostGLAccount(params *JournalPostGLAccountParams, opts ...ClientOption) (*JournalPostGLAccountOK, error) {
	...
	if params == nil {
		params = NewJournalPostGLAccountParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "Journal_PostGLAccount",
		Method:             "POST",
		PathPattern:        "/v1/journals/commands/import",
		...
		Params:             params,
		Reader:             &JournalPostGLAccountReader{formats: a.formats},
	...
	success, ok := result.(*JournalPostGLAccountOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for Journal_PostGLAccount: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
